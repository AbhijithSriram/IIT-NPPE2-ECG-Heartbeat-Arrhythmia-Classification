...
add_md("""# Recording-Aware ECG Classification: Variance-Reduced CNN + Gradient Boosting Blend
...
**What is new here:** the CNN architecture and inputs are held exactly as in the run that
scored 0.62793 on the leaderboard, and only two variance-reducing changes are layered on:
each fold now averages several independently seeded networks (per-fold F1 previously ranged
0.65 to 0.76, which is single-network noise), and predictions are averaged over small
test-time time shifts. The blend weight against the gradient-boosted models is now fitted
**per class** rather than as a single scalar, because the two model families are not
uniformly better or worse than one another - the CNN wins on SVEB while gradient boosting
wins on Fusion.
...
CNN_PATIENCE = 7
CNN_BATCH = 256
CNN_SEEDS = 2      # per-fold CNN variance was large (fold F1 ranged 0.65-0.76); average it out
...
The architecture is deliberately left exactly as it was in the run that scored 0.62793, where
it reached 0.6872 honest OOF against the gradient boosting's 0.6701 and lifted SVEB F1 from
0.558 to 0.665 (recall 0.414 -> 0.591). The only additions here are two changes that reduce
variance rather than alter what the model can represent:

- **Seed averaging.** Per-fold F1 ranged from 0.65 to 0.76, which is single-network noise, not
  signal. Each fold trains `CNN_SEEDS` networks and averages their predictions.
- **Test-time augmentation** over small time shifts.
...
if USE_CNN:
    try:
        ...
        class ECGNet(nn.Module):
            \"\"\"The v4 network, unchanged - this exact architecture produced LB 0.62793.\"\"\"
...
with open("submission_v6.ipynb", "w") as f:
    json.dump(notebook, f, indent=2)
print("Wrote submission_v6.ipynb")
